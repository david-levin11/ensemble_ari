...
ari_dur = 24
ari_area = "ak"
ari_field = f"global_ari_{ari_dur}hr"
ari_array = f'gl{ari_dur}ha'
ari_file = f"{ari_field}.nc"
ari_name = f"{ari_area}{ari_len}yr{ari_dur}ha"
with xr.open_dataset(os.path.join(ensemble_dir, ari_file)) as ds:
    print(ds)
    in_grid = {'lon': ds['lon'].values, 'lat': ds['lat'].values}
    ari = ds[ari_array].sel(ARI=ari_len)
...
print('\n\n')
print(f"ARI field is: {ari_field}")
...
outfilename = os.path.join(output_dir, f"{ari_name}_regridded_to_{base_model}.nc")
