...
base_exceedance_dir = r'C:\Users\David.Levin\ensemble_ari\ensemble_exceedance_data'

base_graphics_dir = r'C:\Users\David.Levin\ensemble_ari\ensemble_exceedance_graphics'

ensemble = 'gefs'

ensemble_shortname = 'gefs'
...
#extent = [-125, -67, 25, 50]
#extent = [-165, -151, 14, 27]
#extent = ([-90, -65, 25, 40])
#plotcrs = ccrs.NorthPolarStereo(central_longitude=-150, true_scale_latitude=60)
#plotcrs = ccrs.Mercator(central_longitude=-160, latitude_true_scale=20)
#plotcrs = ccrs.PlateCarree()
plotcrs = ccrs.LambertConformal(central_longitude=-95, central_latitude=25)
...
forecast_projections = [f"F{hour:03d}" for hour in range(int(ri_duration), max_hour + int(ri_duration), rolling_duration)]
...
for ri in ri_lengths:
    ...
    for tstep in forecast_projections:
        ...
        try:
            ...
            print(f"Can't find {exceedance_file}, in {exceedance_dir}")
